view: event_session_funnel {
  derived_table: {
    sql: SELECT
        ...
        , MIN(
            CASE WHEN
              {% condition first_stage_funnel %} event_type {% endcondition %}
              THEN created_at
              ELSE NULL END
            ) as event1_time
        , MIN(
            CASE WHEN
              {% condition second_stage_funnel %} event_type {% endcondition %}
              THEN created_at
              ELSE NULL END
            ) as event2_time
        , MIN(
            CASE WHEN
              {% condition third_stage_funnel %} event_type {% endcondition %}
              THEN created_at
              ELSE NULL END
            ) as event3_time
      FROM events
 ...
 ;;
  ...
  filter: first_stage_funnel {
    type: string
    default_value: "Product"
    suggest_explore: events
    suggest_dimension: events.event_type
  }

  filter: second_stage_funnel {
    type: string
    default_value: "Cart"
    suggest_explore: events
    suggest_dimension: events.event_type
